...
(define (scm->c port type expr)
  ...
  (cond
   ((or (string= stype "char *") (string= stype "uchar *"))
    (let ((local (genlocal)))
      (format port "    char *~a = scm_is_false(~a) ? NULL : scm_to_utf8_stringn(~a, NULL);\n    if (~a) scm_dynwind_free(~a);\n" local expr expr local local)
      local))
   ...
   ((member dtype struct-names)
    (let ((local (genlocal)))
      (format port "    ~a ~a = NULL;\n" stype local)
      (format port "    if (!scm_is_false(~a)) {\n" expr)
      (format port "        if (SCM_POINTER_P(~a)) {\n" expr)
      (format port "            ~a = (~a)scm_to_pointer(~a);\n" local stype expr)
      (format port "        } else {\n")
      (format port "            scm_assert_foreign_object_type(rgtype_~a, ~a);\n" dtype expr)
      (format port "            ~a = (~a)scm_foreign_object_ref(~a, 0);\n" local stype expr)
      (format port "        }\n")
      (format port "    }\n")
      local))
   ((string= stype "float") (format #f "scm_to_double(~a)" expr))
   ((string-contains stype "*")
    (let ((local (genlocal)))
      (format port "    void *~a = scm_is_false(~a) ? NULL : scm_to_pointer(~a);\n" local expr expr)
      local))
...
(define (c->scm port type expr)
  ...
  (cond
   ((or (string= stype "char *") (string= stype "uchar *"))
    (let ((local (genlocal)))
      (format port "    char *~a = ~a;\n" local expr)
      (format #f "(~a ? scm_from_utf8_string(~a) : SCM_BOOL_F)" local local)))
   ((string= type "void")
   ...
   ((member dtype struct-names)
    (let ((local (genlocal)))
      (format port "    ~a ~a = ~a;\n" stype local expr)
      (format #f "(~a ? scm_make_foreign_object_1(rgtype_~a, ~a) : SCM_BOOL_F)" local dtype local)))
   ((string= stype "float") (format #f "scm_from_double(~a)" expr))
   ((string-contains stype "*")
    (let ((local (genlocal)))
      (format port "    void *~a = (void *)(~a);\n" local expr)
      (format #f "(~a ? scm_from_pointer(~a, NULL) : SCM_BOOL_F)" local local)))
